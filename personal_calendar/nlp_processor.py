...
class NLPProcessor:
    ...
    def _extract_reminder_advanced(self, text: str) -> Tuple[int, str]:
        ...
        patterns = [
            # "nhắc trước 10 tiếng", "báo trước 2 tiếng", "nhắc trước 2 giờ"
            r'(?:nhắc|báo)\s+trước\s+(\d+)\s*(?:tiếng|giờ)',
            # "nhắc 10 tiếng trước", "báo 2 giờ trước"
            r'(?:nhắc|báo)\s+(\d+)\s*(?:tiếng|giờ)\s+trước',
            # "trước 10 tiếng", "trước 2 giờ"
            r'trước\s+(\d+)\s*(?:tiếng|giờ)',
            # "10 tiếng trước", "2 giờ trước"
            r'(\d+)\s*(?:tiếng|giờ)\s+trước',
            # "nhắc trước 15 phút", "báo trước 30 phút"
            r'(?:nhắc|báo)\s+trước\s+(\d+)\s*phút',
            # "nhắc 15 phút trước", "báo 30 phút trước"
            r'(?:nhắc|báo)\s+(\d+)\s*phút\s+trước',
            # "trước 15 phút"
            r'trước\s+(\d+)\s*phút',
            # "15 phút trước"
            r'(\d+)\s*phút\s+trước',
    ...
    def _build_datetime(self, time_info: Dict) -> Tuple[datetime, Optional[datetime]]:
        ...
        # else:
        #     # Mặc định thời gian kết thúc = thời gian bắt đầu + 1 giờ
        #     end_datetime = start_datetime + timedelta(hours=1)
...
if __name__ == "__main__":
    ...
    test_cases = [
        "Nhắc tôi Học bài vào lúc 09:00 ngày mai ở phòng học",
        "Họp team lúc 9h30 sáng mai tại phòng A101 nhắc trước 15 phút",
        "Đi chơi tối thứ 7 này lúc 8 giờ",
        "Học tiếng Anh ngày 10/11 lúc 14:00 ở trung tâm",
        "Họp từ 9h đến 11h ngày mai",
        "Sinh nhật bạn tối nay 7h30 ở quán cafe",
        "Khám bệnh lúc 8h sáng thứ 2 ở bệnh viện",
    ]
